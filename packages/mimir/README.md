...
[`no-unused-label`](docs/no-unused-label.md) | :wrench: Disallws labels that are never used. | TSLint only has `label-position` which doesn't check for unused labels.
